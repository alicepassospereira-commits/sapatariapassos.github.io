...
window.fecharServico = async function(id){
  const docSnap = await getDocs(collection(db, "orcamentos"));
  const docRef = doc(db, "orcamentos", id);
  const docData = (await docRef.get()).data; // alternativa para modular

  const s = (await import("firebase/firestore")).getDoc(docRef);
  const sData = (await s).data();
...
window.marcarPronto = async function(id){
  ...
  const sSnap = await import("firebase/firestore").then(fire => fire.getDoc(docRef));
  const sData = (await sSnap).data();
