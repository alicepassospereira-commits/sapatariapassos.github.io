...
async function renderizar() {
  ...
  for (let docSnap of orcamentosSnap.docs) {
  ...
  listaAF.innerHTML = "";
  const aFazerSnap = await getDocs(collection(db, "a_fazer"));
  for (let docSnap of aFazerSnap.docs) {
  ...
  listaP.innerHTML = "";
  const prontosSnap = await getDocs(collection(db, "prontos"));
  for (let docSnap of prontosSnap.docs) {
...
document.getElementById("formOrcamento").addEventListener("submit", async e=>{
  ...
  if(files.length === 0){
    await addDoc(collection(db, "orcamentos"), {cliente, telefone, servico, valor, entrada, dataEntrega, fotos, status:"orcamento"});
    renderizar();
  } else {
    for (let f of files){
...
window.marcarPronto = async function(id){
  ...
  window.open(https://wa.me/55${sData.telefone}?text=${encodeURIComponent("Olá, aqui é da Sapataria Passos! Seu conserto está pronto!")}, "_blank");
  renderizar();
}
